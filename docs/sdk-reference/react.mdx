...
<ResponseField name="isModal" type="boolean" required>
  When set to `true`, the importer will behave as a modal with its open state controlled by `modalIsOpen`. When set to
  `false`, the importer will be embedded directly in your page.
</ResponseField>

<ResponseField name="modalIsOpen" type="boolean" default={false}>
  Only used when `isModal` is set to `true`: Controls the importer modal being open or closed.
</ResponseField>

<ResponseField name="modalOnCloseTriggered" type="function">
  Only used when `isModal` is set to `true`: A function called when the user clicks the close button or clicks outside of
  (when used with `modalCloseOnOutsideClick`) the importer. `useState` can be used to control the importer modal opening
  and closing.
  ```javascript
  const [isOpen, setIsOpen] = useState(false);
  ```
