...
```javascript TableFlow Open Source
import { useState } from "react";
import { TableFlowImporter } from "@tableflow/react";
...
<ResponseField name="importerId" type="string">
  <Info>Required in <a href="https://tableflow.com/pricing" target="_blank">TableFlow Cloud</a>.</Info>
  The unique ID for the importer. The SDK used this ID to pull in the template schema that you set up in the Admin
  Dashboard, or you can override this by adding the template parameter. Obtain from your importer on
  [TableFlow](https://app.tableflow.com/importers).
</ResponseField>
...
<ResponseField name="template" type="object">
  Configure the columns used for the import. If provided when using TableFlow Cloud, this template will override what is
  defined in your importer on the [TableFlow admin app](https://app.tableflow.com/importers). This can be useful if
  columns you want your users importing change depending on a condition in your application.
...
<ResponseField name="hostUrl" type="string">
  The URL of where the importer iframe is hosted. This should only be set when deploying TableFlow Open Source.
</ResponseField>
