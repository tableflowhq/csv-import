...
func GetDatabaseSchemaInitSQL() string {
	return `
		...
		create table if not exists workspaces (
			...
			allowed_import_domains text[]                   not null default '{}',
			created_by             uuid                     not null,
			created_at             timestamp with time zone not null,
			updated_by             uuid                     not null,
			updated_at             timestamp with time zone not null,
		...
		create table if not exists template_columns (
			...
			index              int, -- The 0-based position of the column
		...
		alter table importers
			drop column if exists webhooks_enabled;

		alter table importers
			drop column if exists skip_header_row_selection;

		alter table importers
			drop column if exists allowed_domains;

		alter table workspaces
			add column if not exists allowed_import_domains text[] not null default '{}';
	`
}
