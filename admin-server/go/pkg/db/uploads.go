...
func GetUpload(id string) (*model.Upload, error) {
	...
	err := DB.Preload("UploadColumns").First(&upload, model.ParseID(id)).Error
	if err != nil {
		return nil, err
	}
	if !upload.ID.Valid {
...
func IsUploadStored(id string) (bool, error) {
	...
	var res Res
	err := DB.Raw("select is_stored from uploads where id = ?;", id).Scan(&res).Error
...
func GetUploadByTusID(tusID string) (*model.Upload, error) {
	...
	var upload model.Upload
	err := DB.First(&upload, "tus_id = ?", tusID).Error
	if err != nil {
		return nil, err
	}
	if !upload.ID.Valid {
	...
	if upload.IsParsed {
		...
		DB.Where("upload_id = ?", upload.ID).Find(&uploadColumns)
